...
export function MembershipAdmin({ onLogout }: OrdersAdminProps) {
  ...
  const filteredOrders = orders.filter((order) => {
    // ONLY show orders that contain a membership
    const hasMembership = Array.isArray(order.items) && order.items.some((item: any) => {
  ...
  return (
    <AdminLayout onLogout={onLogout} currentPage="memberships">
      <div className="admin-page">
        <div className="admin-page-header">
          <h1>Membership Subscriptions</h1>
          <p>Total Memberships: {filteredOrders.length}</p>
        </div>
